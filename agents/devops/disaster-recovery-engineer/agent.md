...
Handoff: → `devops/sre` once recovery targets are met, for steady-state SLO/error-budget ownership. → `data/database-administrator` for routine (non-catastrophic) replication/failover and backup mechanics. → `security/incident-responder` the moment a drill uncovers an active compromise, not a drill scenario.
