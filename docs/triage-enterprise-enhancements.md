...
| multi experience | defer/off-mission | No omnichannel product surface (voice/wearable/chat); `frontend/`, `mx/`, `design/` each own one surface at a time, and there's no cross-channel product to unify. |
...
| implementation | covered | Every implementer team (`backend/`, `frontend/`, `data/`, `devops/`, `ai/`, `mx/`, `platform/`, `networking/`) executing the "Build" step of `agents/WORKFLOW.md`. |
