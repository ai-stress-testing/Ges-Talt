...
`agents/ci/` + `agents/cd/` · MITRE TA0002
...
| T1609 Container Administration Command | Restrict container API (Docker/Kubernetes) access via RBAC; monitor audit logs for privileged exec commands | `agents/cd/orchestration-engineer` | prevent + detect |
| T1610 Deploy Container | Scan images for vulnerabilities before deploy; block privileged/untrusted container deployment via admission controllers | `agents/ci/containerization-engineer` + `agents/cd/orchestration-engineer` | prevent |
...
| T1072 Software Deployment Tools | Secure credentials for SCCM/Intune/Ansible; monitor deployment logs for out-of-policy execution | `agents/cd/gitops-engineer` | prevent + detect |
